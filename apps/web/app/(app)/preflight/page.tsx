...
import { PageShell, CardEntrance, LiftCard } from "@/components/ui/motion";
        ...
        {analyses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {analyses.map((analysis, i) => (
              <CardEntrance key={analysis.id} index={i}>
                <LiftCard>
                  <Link href={`/preflight/${analysis.id}`} className="block">
                    <Card className="cursor-pointer transition-shadow duration-300 hover:shadow-lg hover:shadow-black/[0.07]">
                      <CardHeader className="pb-2">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-sm font-semibold line-clamp-1 tracking-tight">
